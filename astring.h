///*****************************Asalamualaikum Welcome to Awais's string class******************************

/// Copyright (C) 2022 SPARK.   (https://awaisoem.github.io/SPARK-OFFICIALS/)
///************NOTE:: CURRENTLY WORKING ON THIS SO THIS IS NOT COMPLETE VERSION (matlab kuch kehne se pehle se parh lein phir bhi agar criticize karna hai to bhar me jaein)*****************

/// *****************************Features*****************************
///   ->Use Dynamic Memory
///   ->HAve Some Pythonic Functions
///   ->Provide More feactures then normal string
///   ->His Functions Compatible with cstring, normal string and itself
/// *****************************Funtions*****************************
///   ->assign(cstring),operator = (these for assigning values)
///   ->append(cstring|normal string|itself),operator += (these for append string)
///   ->insert(index  , cstring|normal string|itself) {this will insert any string into given index Note: Length Changes Dynamically}
...
///                 currently working on FIXME: cstring +cstring
/// erase(index,length) {erase string of length l from given index}
/// replace(index,length,cstring|normal string|itself) {replace string of length from given index and insert new at that index}
/// push_back(char)  add one character at end
/// pop_back(void)  remove last index
/// center(length,char)  pythonic function that center the string in given length and fill other positions with character
/// count(char)   pythonic function cout total occurence of character in string
/// substr(start index, end index)  give sub string
/// swap(itself) swap with another string
/// reverse(void)     reverse the string
/// globel getline(input stream,string,character)    for getting line orr multiple lines
///   copy(new string,start index,end index)       this will copy string from start index to end index to given string
///  at(index) return value at that index
/// find(searching chartacter,starting index(default 0))    find from starting index to end and return index of character
/// rfind(searching chartacter,starting index(default last))    reverse find from starting index to begening and return index of character
...
/// capatilize() convert sentence's first character to upper for all sentences in string
/// operator [] for access to indexs
/// size(),length() give length of string
/// capacity() return capacity of one buffer used in dynamic memory allocation
/// max_size() return maximum size of string can acheive actually (buffer_capacity*number_of_buffers)


/// Developer:: AWAIS RAZA (http://awaisoem.netlify.app/)
#pragma once
#include <bits/stdc++.h>
using namespace std;
